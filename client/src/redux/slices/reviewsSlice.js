...
export const getReviews = () => async (dispatch) => {
  try {
    const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/comments`);
    console.log(data);
...
export const addNewReview = (review) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`${process.env.REACT_APP_API_URL}/api/comments`, state.reviews.newReview, {
      headers: {
...
export const removeReviewId = (_id) => async (dispatch) => {
  try {
    await axios.delete(`${process.env.REACT_APP_API_URL}/api/comments/_id`);
