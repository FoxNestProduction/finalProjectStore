...
export const addNewReview = (review) => async (dispatch) => {
  try {
    const { data } = await axios.post('http://localhost:4000/api/comments', review);
    console.log(data);
    dispatch(addReview(review));// eslint-disable-line no-use-before-define
  } catch (error) {
...
export const { setReviews, addReview, removeReview } = reviewsSlice.actions;
